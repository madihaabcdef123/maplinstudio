        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="statusMsg"></p>
                    ...
                    @auth
                     <input type="hidden" id="job_id" name="user_id" value="{{Auth::user()->id}}" >
                     @else
                     <input type="hidden" id="job_id" name="user_id" value="0" >
                     @endauth
                    <div class="form-group">
                        <label for="inputName">Name</label>
                        <input type="text" class="form-control" id="inputName" name="name" value="{{(isset($user)?$user->name:'')}}" placeholder="Enter your name"/>
                    </div>
                    <div class="form-group">
                        <label for="inputEmail">Email</label>
                        <input type="email" class="form-control" name="email"  value="{{(isset($user)?$user->email:'')}}" id="inputEmail" placeholder="Enter your email"/>
                    </div>

                    <div class="form-group">
                        <label for="inputEmail">Contact Number</label>
                        <input type="number" class="form-control" name="contactno"  value="{{(isset($user)?$user->phonenumber:'')}}" id="inputNumber" placeholder="Enter your Contact Number"/>
                    </div>
...
$("#jobApply").submit(function(e) {
            ...
            $.ajax({
                ...
                if(data.status== true){
                  ...
                  window.location ="{{url('thank-you-upload')}}";
              }
                },
                cache: false,
                contentType: false,
