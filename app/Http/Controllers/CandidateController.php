     ...
     public function apply_job(Request  $request)
        {
         ...
         if (Auth::check()){
          ...
          $applied_jobs= new applied_job;
          $applied_jobs->name=$request->name;
          $applied_jobs->email=$request->email;
          $applied_jobs->contactno=$request->contactno;
          $applied_jobs->user_id=$request->user_id;
          $applied_jobs->job_id=$request->job_id;
          $filename =$request->file('resume')->getClientOriginalName();
          $path = $request->file('resume')->storeAs('resume', $filename, 'public');
          $applied_jobs->resume =$path;
          $applied_jobs->save();
          
           }
     return response()->json(['status'=>true,"redirect_url"=>url('thank-you-upload')]);     
