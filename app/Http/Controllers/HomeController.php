...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $orders = orders::with("user")->has('user')->where('is_active', 1)->where('paid_status', 'Paid')->orderBy("id", "desc")->get();
        // Balance Sheet
        $balance = 0;
        if ($orders) {
        ...
        $previous_balance = orders::with("user")->has('user')->select('amount')->where('is_active', 1)->where('paid_status', 'Paid')->limit(10)->latest('created_at')->get()->pluck('amount')->toArray();
        ...
        // Refund Cases
        $previous_balance_refund = orders::with("user")->has('user')->select('amount')->where('is_active', 1)->where('paid_status', 'Refund')->limit(10)->latest('created_at')->get()->pluck('amount')->toArray();
        $balance_refund = 0;
        ...
        // Payment Schdule
        $previous_balance_date = orders::with("user")->has('user')->where('is_active', 1)->where('paid_status', '!=', 'Pending')->get();
        $date_array = array();
...
// file details end
    ...
    public function newsletter_manage()
    {
        ...
        return view('newsletter-manage')->with('title', "Newsletter Management")->with(compact('user', 'newsletter'));
    }
    ...
    public function modalform(Request $request)
    {
        ...
        $class = $_POST['class'];
        $tag = $_POST['tag'];
        $wow_delay = $_POST['wow_delay'];
        $wow_duration = $_POST['wow_duration'];
        $body="";
        try{
                                ...
                                <div class="modal-body">
                                    <form class="" id="cms_form" method="POST" action="'.$route_url.'">
                                        <div class="row">
                                            ...
                                            <input type="hidden" name="wow_duration" id="slug" value="'.$wow_duration.'">
                                            <input type="hidden" name="wow_delay" id="slug" value="'.$wow_delay.'">
    ...
    public function upload_gallery($model='',$id='')
    {
        if ($id == '') {
        ...
        $gallery = breakdown_img::where("is_active" ,1)->where("model" , $model)->where("is_deleted" , 0)->where("record_id" , $project->id)->get();

        return view("upload-gallery")->with(compact('project','gallery','model'));
    }

    public function images_submit(Request $request) {
        ...
        if (isset($_POST['record_id']) && $_POST['record_id'] != 0) {
            ...
            if ($request->hasFile('file')) {
                foreach ($request->file as $key => $img_path) {
                    ...
                    $path = $img_path->storeAs('uploads/'.$_POST['model'], $filename, 'public');
                    $ext = pathinfo($filename, PATHINFO_EXTENSION);
                    
                    if ($ext == "png" || $ext == "jpeg" || $ext == "jpg") {
                        $breakdown_img = new breakdown_img;
                        $breakdown_img->record_id = $_POST['record_id'];
                        $breakdown_img->model = $_POST['model'];
   ...
   public function applied_job_view()
      {

        $applied_jobs=applied_job::with('jobs')->where('is_active',1)->where('is_deleted',0)->get();
     
          
        return view('display-applied-jobs')->with(compact('applied_jobs'));
     }
