...
class Helper
{
    ...
    public static function invoice_num($val = '')
    {
        $order_num = sprintf("%04d", $val);
        return $order_num;
    }



    public static function editck($t, $class, $wow_delay, $wow_duration, $desc, $slug )
    {
        $content = web_cms::where("slug",$slug)->orderBy('id','desc')->first();
        if (Auth::user() && Auth::user()->role_id ==1) {
            if ($content) {
                $body = '<div class="'.$class.' clickable" data-wowdelay = "'.$wow_delay.'" data-wowduration = "'.$wow_duration.'" data-slug="'.$slug.'" data-class="'.$class.'" data-tag="'.$t.'">'.$content->desc.'</div>';
            } else{
                $body = '<div class="'.$class.' clickable" data-wowdelay = "'.$wow_delay.'" data-wowduration = "'.$wow_duration.'" data-slug="'.$slug.'" data-class="'.$class.'" data-tag="'.$t.'"><'. $t .' class="'.$class.'" data-wow-delay = "'.$wow_delay.'" data-wow-duration = "'.$wow_duration.'"> '.$desc.' </'. $t .'></div>';
            }
        } else{
            if ($content) {
                $body = '<div class="'.$class.'" data-wowdelay = "'.$wow_delay.'" data-wowduration = "'.$wow_duration.'"  >'.$content->desc.'</div>';
            } else{
                $body = '<'. $t .' class="'.$class.'" data-wow-delay = "'.$wow_delay.'" data-wow-duration = "'.$wow_duration.'"> '.$desc.' </'. $t .'>';
            }
        }
        return $body;
    }
